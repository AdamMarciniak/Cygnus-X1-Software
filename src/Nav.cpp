...
void getInitYawAndPitchBiases() {
  ...
  // pitchBiasQuaternion = from_axis_angle(data.pitchBias * DEG_TO_RAD, 0, 1,
  // 0);
}
...
void getYPR() {
  ...
  if (!first_gyro_reading) {
    ...
    q_body[0] = q_gyro[0] * q[0] - q_gyro[1] * q[1] - q_gyro[2] * q[2] -
                q_gyro[3] * q[3];
    q_body[1] = q_gyro[0] * q[1] + q_gyro[1] * q[0] + q_gyro[2] * q[3] -
                q_gyro[3] * q[2];
    q_body[2] = q_gyro[0] * q[2] - q_gyro[1] * q[3] + q_gyro[2] * q[0] +
                q_gyro[3] * q[1];
    q_body[3] = q_gyro[0] * q[3] + q_gyro[1] * q[2] - q_gyro[2] * q[1] +
                q_gyro[3] * q[0];
    ...
    orientation *= from_axis_angle(gyro_dt * norm, omega[0] / norm,
                                   omega[1] / norm, omega[2] / norm);
...
void quatToEuler(float *qBody, float *ypr) {
  ...
  double cosr_cosp =
      1.0 - 2.0 * (q_body[1] * q_body[1] + q_body[2] * q_body[2]);
  ...
  double cosy_cosp =
      1.0 - 2.0 * (q_body[2] * q_body[2] + q_body[3] * q_body[3]);
