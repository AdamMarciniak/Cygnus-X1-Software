...
bool SPIFlash::_prep(uint8_t opcode, uint32_t _addr, uint32_t size)
{
  ...
  {
    if (!_enable4ByteAddressing())
    { // If unable to enable 4-byte addressing
      return false;
...
void SPIFlash::_endSPI(void)
{
  ...
  if (address4ByteEnabled)
  { // If the previous operation enabled 4-byte addressing, disable it
...
{
  if (!(_readStat3() & ADS))
  { // If 4 byte addressing is disabled (default state)
    return true;
  }
