...
int LSM9DS1Class::begin()
{
  ...
  byte whoami = readRegister(LSM9DS1_ADDRESS, LSM9DS1_WHO_AM_I);
  Serial.println(whoami);
  if (readRegister(LSM9DS1_ADDRESS, LSM9DS1_WHO_AM_I) != 0x68) {
    end();
    Serial.println("Wrong ACCEL GYRO WHOAMI");
  ...
  if (readRegister(LSM9DS1_ADDRESS_M, LSM9DS1_WHO_AM_I) != 0x3d) {
    ...
    Serial.println("Wrong MAG WHOAMI");


    return 0;
  }

  writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG1_G, 0x57); // 119 Hz, 250 dps, 16 Hz BW
