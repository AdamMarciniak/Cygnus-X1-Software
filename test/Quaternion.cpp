...
//Converting from quaternion to roll pitch yaw figure this out later
...
//         ret.x() = atan2(2 * (_w * _x + _y * _z), 1 - 2 * (sqx + sqy)) * RAD_TO_DEG;

//         float sinp = 2 * (_w * _y - _x * _z);
//         if(sinp >= 1) ret.y() = 90;
//         else if(sinp <= -1) ret.y() = -90;
//         else ret.y() = asin(sinp) * RAD_TO_DEG;

//         ret.z() = atan2(2 * (_w * _z + _x * _y), 1 - 2 * (sqy + sqz));
